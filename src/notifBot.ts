...
export const notifBot = new Bot<CustomContext>(process.env.BOT_TOKEN || '')

notifBot.use(useFluent({
  fluent,
}));

// 2. Attach an api throttler transformer to the bot
notifBot.api.config.use(apiThrottler())
...
// 6. Start the bot
// This command has the side effect of removing the Telegram WebHook
// notifBot.start()
